...
var HashRing = require('hashring')
  ...
  //meh, Jackpot = require('jackpot')

  , Utils = require('./utils')
  , IssueLog = Connection.IssueLog;

var DaddyPool = require('./daddypool.js');
...
function Client (args, options) {
  ...
  if (typeof args !== 'string'){
      new Error("Multiple servers no longer supported. (omgpop) ");
  }

  var tokenize = args || regular;
  var tokens = tokenize.split(':');
  if (tokens.length!=2){
      new Error("Expecting string format of ADDRESS:PORT");

  }
  this.daddyPool = new DaddyPool(tokens[0], tokens[1],this.poolSize);
...
//    }
    ...
    this.daddyPool.get(augmentSocket.bind(this, stuff));
  };

  function augmentSocket(stuff, err, S){
      if (S){
          S.setTimeout(stuff.memcached.timeout);
          S.setNoDelay(true);
          S.setEncoding('utf8');
          S.metaData = [];
          S.responseBuffer = "";
          S.bufferArray = [];
          S.serverAddress = stuff.server;
          S.port = stuff.port;
          S.tokens = [].concat(stuff.serverTokens);
          S.memcached = stuff.memcached;
          Utils.fuse(S, {
