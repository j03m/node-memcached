...
Daddypool.prototype.nextAvailableConnection= function nextAvailableConnection(){
    for (var i=0; i<this.availablePool.length; i++){
        if (this.availablePool[i].status == 1){
            return this.availablePool[i];
        }else{
            if (this.isAvailable(this.availablePool[i].socket)){
                return this.availablePool[i];
            }
        }
    }
...
Daddypool.prototype.isAvailable = function isAvailable(socket){
    var readyState = socket.readyState
        , writable = readyState === 'open' || readyState === 'writeOnly'
        , writePending = socket._pendingWriteReqs || 0
        , writeQueue = socket._writeQueue || []
...
Daddypool.prototype.doConnectAndCallback = function doConnAndCall(connection, request){
    if (this.isConnected(connection.socket)){
        ...
        //connection.socket.connect({path: '/var/run/mcmux/mcmux.sock'});
        connection.socket.connect({path: '/Users/jmordetsky/mcmux/mcmux.sock'});

        connection.socket.on('connect', function(inRequest, inConnection, pool){
                return function(){
                    //console.log('DaddyPool:  connect');
                    inConnection.socket.oldWrite = inConnection.socket.write;
                    inConnection.socket.write = write.bind(inConnection.socket);
                    inConnection.socket.poolAddress  = pool.address;
                    inConnection.socket.poolPort  = pool.port;
                    inRequest.callback(null, inConnection.socket, inConnection.id);
                }
            }(request, connection, this)).on('close',function(){
...
//write
function write(data){
    return this.oldWrite('A:'+this.poolAddress+":"+this.poolPort+" "+data);
}
